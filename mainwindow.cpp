...
void MainWindow::on_addBtn_clicked()
{
    ...
    if(!name.isEmpty() && !id.isEmpty()) {
        manager.addStudent(name.toStdString(), id.toStdString()); //tostdstring converts the Qt-specific string into a format your Student constructor understands
...
void MainWindow::on_updateGradeBtn_clicked()
{
    ...
    if (manager.studentExist(id.toStdString())) {
        ...
        double newGPA = s.getInternalGPA(); // instead of calling calculate gpa i just fetched it since it's calculated in update grade auto
...
void MainWindow::displayStudentReport(const Student& s) {
    ...
    html += "<h3 style='color: " + gpaColor + ";'>Current GPA: " +
            QString::number(s.getInternalGPA()) + "</h3>";
...
void MainWindow::on_subAvgBtn_clicked()
{
    ...
    if (result != "EMPTY") {
        ...
        html += "<p style='font-size: 16px;'>None of the students registered for <b>" + subject + "</b> yet.</p>";
    }
